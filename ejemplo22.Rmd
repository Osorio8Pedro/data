...
first_date <-"2022-02-06"
last_date<-"2022-03-13"
...
```{r, incidence2, message=FALSE,echo=FALSE, results='asis',warning=F}
...
c=(round(log10(ftregion$cant_casosconfirmadosdiario),5))#*100
c=10^(ftregion$cant_casosconfirmadosdiario)
...
v=(round(log10(ftregion$cant_casosconfirmados),5))#*100
v=10^(ftregion$cant_casosconfirmados)
...
dat<-filter(df_state,state=='00. Nacional')# nhs_region=='17. Norte Grande' |nhs_region=='18. Norte Chico' | nhs_region=='19. Zona Central' |nhs_region=='20. Zona Sur'| nhs_region=='21. Zona Austral'|nhs_region=='22. Regions Sin Metropolitana')
...
df_all_plot <- do.call(dplyr::bind_rows, list(df_all_raw, df_all_adjusted))
 ...
 df_all_plot$type<- unique(df_all_plot$type)
df_type <- df_all_plot[df_all_plot$type == type, ]
sort(unique(df_type$level))
...
df_state$New_Deaths_max<-(round(log10(df_state$New_Deaths_max),3))
df_state$midline<-(round(log10(df_state$midline),3))
df_state$lcl<-(round(log10(df_state$lcl),3))
df_state$ucl<-(round(log10(df_state$ucl),3))

df_all$New_Deaths<-(round(log10(df_all$New_Deaths),3))
df_all$New_Deaths_max<-(round(log10(df_all$New_Deaths_max),3))
...
tabla=df_all[which(df_all$datex>"2021-12-02"),]
tabla=tabla[which(tabla$datex<"2021-12-04"),]

library(xtable)
DATA2=tabla %>% select(statephase,epoch)
xtable(DATA2)
##
```
